"""Evaluate ArcFace models using in-memory gallery embeddings.
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Evaluate ArcFace models on aligned 112x112 gallery/test folders without detection or FAISS"
    )
    ...
    parser.add_argument(
        "--output-csv",
        type=str,
        default=os.path.join(PROJECT_ROOT, "evaluation", "arcface_eval_results.csv"),
...
def embedding_from_aligned_image(recognizer: ArcFace, image_bgr: np.ndarray) -> np.ndarray:
...
def build_gallery_embeddings(recognizer: ArcFace, gallery_dir: str, aggregation: str):
    gallery_store = {}
    ...
    for sample in samples:
        ...
        try:
            image = read_aligned_face(image_path, expected_size=recognizer.input_size)
...
def main():
    ...
    logging.info(f"Loading ArcFace recognizer from: {params.rec_weight}")
    ...
    print("\n" + "=" * 60)
    print("ArcFace evaluation summary")
    print("=" * 60)
