"""Evaluate FocusFace using in-memory gallery embeddings.
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Evaluate FocusFace on aligned 112x112 gallery/test folders without detection or FAISS"
    )
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Path to FocusFace checkpoint, e.g. ./weights/focus_face_w_pretrained.mdl"
    )
    parser.add_argument(
        "--gallery-dir",
        type=str,
    ...
    parser.add_argument(
        "--output-csv",
        type=str,
        default=os.path.join(PROJECT_ROOT, "evaluation", "focusface_eval_results.csv"),
...
def embedding_from_aligned_image(recognizer: FocusFaceEncoder, image_bgr: np.ndarray) -> np.ndarray:
    image_rgb = cv2.cvtColor(image_bgr, cv2.COLOR_BGR2RGB)
    pil_img = Image.fromarray(image_rgb)
    embedding = recognizer.get_embedding_from_pil(pil_img)
...
def build_gallery_embeddings(recognizer: FocusFaceEncoder, gallery_dir: str, aggregation: str):
    gallery_store = {}
...
def main():
    ...
    logging.info(f"Loading FocusFace recognizer from: {params.rec_weight}")
    ...
    try:
        recognizer = FocusFaceEncoder(params.rec_weight)
    except Exception as e:
    ...
    print("\n" + "=" * 60)
    print("FocusFace evaluation summary")
    print("=" * 60)
