"""Evaluate MaskInv using in-memory gallery embeddings.
...
from models.maskinv import MaskInv
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Evaluate MaskInv student on aligned 112x112 gallery/test folders without detection or FAISS"
    )
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Path to MaskInv ONNX model, e.g. ./weights/maskinv/maskinv_hg.onnx"
    )
    parser.add_argument(
        "--gallery-dir",
        type=str,
    ...
    parser.add_argument(
        "--output-csv",
        type=str,
        default=os.path.join(PROJECT_ROOT, "evaluation", "maskinv_eval_results.csv"),
...
def embedding_from_aligned_image(recognizer: MaskInv, image_bgr: np.ndarray) -> np.ndarray:
...
def build_gallery_embeddings(recognizer: MaskInv, gallery_dir: str, aggregation: str):
    gallery_store = {}
...
def main():
    ...
    logging.info(f"Loading MaskInv student from: {params.rec_weight}")
    ...
    try:
        recognizer = MaskInv(params.rec_weight)
    except Exception as e:
    ...
    print("\n" + "=" * 60)
    print("MaskInv student evaluation summary")
    print("=" * 60)
