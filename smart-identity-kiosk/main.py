"""
Legacy/live recognition entry point for SmartIdentity.

Status: adapted from external source.
External reference: yakhyo/face-reidentification.
Student modification: substantially modified for SmartIdentity project
requirements, including multi-recognizer mode selection,
MaskInv/AdaFace/FocusFace comparison paths, mock airport passenger data joining,
kiosk POST integration and evaluation-oriented database handling.

External baseline reference:
- https://github.com/yakhyo/face-reidentification

See THIRD_PARTY_ATTRIBUTION.md and CODE_OWNERSHIP_MAP.md for source ownership
and licence notes.
"""
...
def get_model_tag(params: argparse.Namespace) -> str:
    ...
    if mode == "arcface":
        rec_weight = os.path.basename(str(params.rec_weight)).lower()

        if "r50" in rec_weight:
            return "arcface_r50"

        if "mbf" in rec_weight or "mobile" in rec_weight:
            return "arcface_mbf"

        # Safe fallback for unknown ArcFace weight name
        return "arcface"
