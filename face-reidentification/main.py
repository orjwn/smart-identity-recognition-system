...
def parse_args():
    ...
    # MaskInv ONNX 
    parser.add_argument("--maskinv-weight", type=str, default="./weights/maskinv/maskinv_hg.onnx",
                        help="Path to MaskInv student ONNX")
    parser.add_argument("--adaface-weight",type=str, default="./weights/adaface_ir18_webface4m.ckpt", help="Path to AdaFace R18 checkpoint"
)
    # mode selection 
    parser.add_argument("--mode", type=str, default="arcface",
                    choices=["arcface", "maskinv", "adaface", "hybrid"],
    ...
    # DB root 
    parser.add_argument("--db-root", type=str, default="./database/face_database",
                        help="DB root folder; will use <db-root>/arcface or <db-root>/maskinv")
    ...
    parser.add_argument("--kiosk-api", type=str, default="http://127.0.0.1:8000",
    ...
    parser.add_argument("--min-post-interval", type=float, default=1.5,
                        help="Seconds between repeated posts for same identity")
    parser.add_argument("--device-id", type=str, default="laptop-prototype",
                        help="Device ID sent to the kiosk backend")
    parser.add_argument("--disable-kiosk-post", action="store_true",
                        help="Disable POSTing recognition events to kiosk backend")
    parser.add_argument("--arcface-high-thresh", type=float, default=0.60,
                    help="ArcFace confident acceptance threshold in hybrid mode")
    parser.add_argument("--arcface-low-thresh", type=float, default=0.40,
                        help="ArcFace lower bound for uncertain band in hybrid mode")
    parser.add_argument("--adaface-thresh", type=float, default=0.50,
...
def resolve_mode(mode: str) -> str:
    ...
    if mode not in ("arcface", "maskinv", "adaface", "hybrid"):
        mode = "arcface"
    return mode
...
def get_model_tag(params: argparse.Namespace) -> str:
    ...
    if mode == "arcface":
        model_file = os.path.basename(params.rec_weight)
        model_tag = os.path.splitext(model_file)[0]
        return f"arcface_{model_tag}"

    if mode == "maskinv":
        model_file = os.path.basename(params.maskinv_weight)
        model_tag = os.path.splitext(model_file)[0]
        return f"maskinv_{model_tag}"

    if mode == "adaface":
        model_file = os.path.basename(params.adaface_weight)
        model_tag = os.path.splitext(model_file)[0]
        return f"adaface_{model_tag}"
...
def build_face_database_for_mode(detector: "SCRFD",
    ...
    params.mode = mode
    try:
        return build_face_database(detector, arc, mki, ada, params, force_update=force_update)
    finally:
...
def build_face_database(detector: "SCRFD",
    ...
    DB stored at:
      <db-root>/arcface  or  <db-root>/maskinv
    ...
    for item_name in os.listdir(params.faces_dir):
        ...
        for filename in os.listdir(person_dir_path):
            ...
            try:
                ...
                embedding = get_embedding(mode, arc, mki, ada, image, kpss[0])
            ...
            try:
                *bbox_coords, _conf = bbox.astype(np.int32)
                ...
                if mode == "hybrid":
                    ...
                    emb = get_embedding(mode, arc, mki, ada, frame, kps)
            ...
            if name != "Unknown":
                ...
                if not params.disable_kiosk_post and debouncer.update(name):
                    ...
                    post_json(
                        ...
                        {"name": name, "similarity": float(similarity), "device_id": params.device_id, "mode": mode}
                    )
...
def main(params):
    ...
    passports_by_name = {normalize_name(p.get("full_name", "")): p for p in passports if p.get("full_name")}
    boarding_by_passport = {bp.get("passport_number"): bp for bp in boarding_passes if bp.get("passport_number")}
    flights_by_number = {f.get("flight_number"): f for f in flights if f.get("flight_number")}

    logging.info(f"Loaded passports: {len(passports)} | boarding_passes: {len(boarding_passes)} | flights: {len(flights)}")

    mode = resolve_mode(params.mode)

    if mode == "hybrid":
        arc_db = build_face_database_for_mode(detector, arc, mki, ada, params, mode="arcface",
                                              force_update=params.update_db)
        ada_db = build_face_database_for_mode(detector, arc, mki, ada, params, mode="adaface",
                                              force_update=params.update_db)
        face_db = None
    else:
        face_db = build_face_database(detector, arc, mki, ada, params, force_update=params.update_db)
        arc_db = None
        ada_db = None
